...
class MbMenu extends CMenu {
    ...
    protected function renderMenuRecursive($items) {
        ...
        foreach ($items as $item) {
            ...
            if (isset($item['url']))
                echo CHtml::link('<span class="icon-medium ' . $item['icon'] . '"></span> ' . $item['label'] . ' '.$totalCount, $item['url'], isset($item['linkOptions']) ? $item['linkOptions'] : array());
            else
                echo CHtml::link('<span class="icon-medium ' . $item['icon'] . '"></span> ' . $item['label'] . ' '.$totalCount, "javascript:void(0);", isset($item['linkOptions']) ? $item['linkOptions'] : array());
