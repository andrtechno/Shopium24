...
class AdminMenu extends CMenu {
    ...
    protected function renderMenuRecursive($items) {
        ...
        foreach ($items as $item) {
            $count = (isset($item['count']) && $item['count'] > 0) ? '<span class="label label-success">' . $item['count'] . '</span>' : '';
            /* if ($this->totalCount) {
            ...
            if (isset($item['url']))
                echo Html::link('<i class="' . $item['icon'] . '"></i> ' . $item['label'] . ' ' . $count, $item['url'], isset($item['linkOptions']) ? $item['linkOptions'] : array());
            else
                echo Html::link('<i class="' . $item['icon'] . '"></i> ' . $item['label'] . ' ' . $count . '<span class="caret"></span>', "javascript:void(0);", isset($item['linkOptions']) ? $item['linkOptions'] : array('class' => 'dropdown-toggle', 'data-toggle' => "dropdown"));
