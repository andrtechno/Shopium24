...
class MultiFileUpload extends CMultiFileUpload {
    ...
    public function run() {
        ...
        /*Yii::app()->controller->widget('ext.bootstrap.fileinput.Fileinput',array(
            'model'=>$name,
    ...
    public function registerClientScript() {
        ...
        if (is_dir($assets)) {
            $cs->registerScriptFile($baseUrl . '/multifile.js');
        }else
            throw new Exception(Yii::t('multifile - Error: Couldn\'t find assets folder to publish.'));

        $cs->registerScript('Yii.MultiFileUpload#' . $id, "jQuery(\"#{$id}\").MultiFile({$options});");
    }
