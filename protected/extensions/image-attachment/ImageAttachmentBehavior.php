...
class ImageAttachmentBehavior extends CActiveRecordBehavior {
    ...
    public function attach($owner) {
        ...
        if (!isset($this->versions['preview']))
            $this->versions['preview'] = array('thumb' => array($this->previewWidth, $this->previewHeight));
    ...
    public function setImage($path) {
        ...
        foreach ($this->versions as $version => $actions) {
          ...
          //  $image = Yii::app()->image->load($path);
           ...
           $image=  Yii::app()->img->load($path);
            //$image2= $img->save($path);
...
//print_r($method);
       ...
       public function setImage2($path) {
        $this->checkDirectories();
        //create image preview for gallery manager
        foreach ($this->versions as $version => $actions) {
            /** @var Image $image */
    ...
    public function updateImages($oldExt = null) {
        if ($this->hasImage($oldExt)) {
            $this->checkDirectories();
            if ($oldExt !== null) {
                $image = Yii::app()->img->load($this->getFilePath('original', $oldExt));
