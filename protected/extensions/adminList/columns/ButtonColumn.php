...
class ButtonColumn extends CButtonColumn {
    ...
    protected function renderDataCellContent($row, $data) {
        ...
        if (!$this->group) {
            ...
            foreach ($this->buttons as $id => $button) {
                ...
                if (isset($data->{hidden_ . $id})) {
                    //if (in_array($data->primaryKey, $this->hidden[$id])) {
                    if (in_array($data->primaryKey, $data->{hidden_ . $id})) {
                        $hidden[] = "{" . $id . "}";
                    }
                }
    ...
    protected function renderButton($id, $button, $row, $data) {
        ...
        if (!isset($options['title']))
            $options['title'] = $label;
        // print_r($button);die();
//iconClass = (isset(Yii::app()->settings->get('app', 'core_tableToolbarClass'))) ? Yii::app()->settings->get('app', 'core_tableToolbarClass') : 'tablectrl_xlarge2';

        $options['class'] = $options['class'] . ' btn btn-default ' . Yii::app()->settings->get('app', 'btn_grid_size');
        ...
        if (isset($button['icon']) && is_string($button['icon'])) {
            if ($id == 'switch') {
                if ($data->switch == 1) {
                    ...
                    $options['class'] = 'switch btn btn-default ' . Yii::app()->settings->get('app', 'btn_grid_size');
                }
            }
    ...
    public function renderFilterCell() {
        if ($this->resetFilter) {
            ...
            echo CHtml::ajaxLink('<i class="flaticon-refresh"></i>', '/' . Yii::app()->controller->route, $ajax, array('class' => 'refresh-filter btn btn-default _btn-xs','id'=>'resetGridFilter','title'=>Yii::t('GridView.default','REFRESH_FILTER')));
