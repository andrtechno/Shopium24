...
class CurrencyCommand extends CConsoleCommand {
    ...
    public function run($args) {
        Yii::log('GetCurrencyCommand job started', 'info', 'cron');
        $this->initVars();
        Yii::log('Getting file', 'info', 'cron');
        if (!$this->downloadPage()) {
            Yii::log('Cannot get file, check internet connection', 'info', 'cron');
            return;
        }else
            Yii::log('File received', 'info', 'cron');;

        Yii::log('Parsing source', 'info', 'cron');
        if (!$this->parcePage()) {
            Yii::log('Cannot parce source', 'info', 'cron');
            return;
        } else
            Yii::log('Data is selected', 'info', 'cron');
        Yii::log('Processing', 'info', 'cron');
        if (!$this->updateCurrency()) {
            Yii::log('Currensy is not updated', 'info', 'cron');
            return;
        } else
            Yii::log('Currency updated', 'info', 'cron');
        Yii::log('Complete', 'info', 'cron');
    }
    ...
    private function parcePage() {
        ...
        if (preg_match_all($this->_regexp, $this->_src_text, $matches)) { // search by text
            ...
            Yii::log('Cannot select data', 'info', 'cron');
        return $result;
    }
