...
<?php if ($items['modules'] !== array()) { ?>
    ...
    <?php foreach ($items['modules'] as $moduleName => $moduleItems) {
       ...
       // $modInfo  = Yii::app()->getModule($moduleName)->name;

       // print_r($modInfo);
        //if(isset(Yii::app()->getModule($moduleName)->name)){
        //    $name = Yii::app()->getModule($moduleName)->name;
       // }else{
            $name = 'unk';
      //  }
        ?>
        <tr><th class="text-center" colspan="3"><?php echo ucfirst($moduleName) . 'Module'; ?> (<?=$name?>)</th></tr>
        <?php
