...
<script>
    ...
    function doTask(taskNum, next, i, mod, num) {
        ...
        setTimeout(function () {
            $.ajax({
                type: "POST",
                url: "/admin/core/translates/ajaxApplication?lang=" + lang,
                data: {
                    file: taskNum[i].file,
                    path: taskNum[i].path
                },
                ...
                success: function (data) {
                    ...
                    $("#sended").text(num - 1 + 1);
                    $('#result tbody #row' + num).html('<td>' + taskNum[i].file + ' </td><td class="text-center">' + mod + '</td><td class="text-center"><span class="label label-' + label + '">' + data.message + '</span></td>');
                    next();
                },
                beforeSend: function () {
