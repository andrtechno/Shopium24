...
class CMapWidget extends CWidget {
    ...
    protected function getMapMarkers(ContactsMaps $model) {
        ...
        foreach ($model->markers as $marker) {
            ...
            $ballproperties = array();
            if (!empty($marker->balloon_content_body)) {
                $ballproperties['balloonContent'] = Yii::app()->controller->renderPartial('mod.contacts.widgets.map.views.balloon', array('data' => $marker), true, false);
                $ballproperties['iconContent'] = $marker->icon_content;
                $ballproperties['hintContent'] = $marker->hint_content;
            }
            //$ballproperties['balloonContentHeader'] = $marker->balloon_content_header;
            //$ballproperties['balloonContentBody'] = $marker->balloon_content_body;
            //$ballproperties['balloonContentFooter'] = $marker->balloon_content_footer;
            $result[] = array(
