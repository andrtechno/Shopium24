...
class FormInputElement extends CFormInputElement {
    ...
    public function renderLabel() {
        ...
        if (isset($this->labelHint)) {
            $hint = '<div class="help-block">' . $this->labelHint . '</div>';
        } else {
            $hint = '';
        }
        if (isset($this->labelHelp)) {
            $hint = '<i class="label-help flaticon-info-circle" data-toggle="popover" title="'.$this->getLabel().'" data-placement="bottom" data-content="' . $this->labelHelp . '"></i>';
        } else {
            $hint = '';
        }
        return CHtml::activeLabel($this->getParent()->getModel(), $this->name, $options) . $hint;
    }
    ...
    public function setClassInput($method) {
        if (Yii::app()->controller instanceof AdminController) {
            ...
            } elseif (strpos($method, 'List') !== false) {
                if (isset($this->attributes['class'])) {
    ...
    public function renderInput() {
        ...
        if (isset(self::$coreTypes[$this->type])) {
            ...
            if (strpos($method, 'List') !== false) {
                return Html::$method($model, $name, $this->items, $this->attributes) . $this->afterField;
            } else {
                return Html::$method($model, $name, $this->attributes) . $this->afterField;
            }
        }
        else {
